...
function ComposingScreen() {
  ...
  return (
    <main className="relative flex min-h-screen flex-col items-center overflow-hidden px-6 pt-16 pb-20">
      ...
      <section className="relative z-10 mt-16 w-full max-w-2xl rounded-lg border-2 border-dashed border-slate-300 bg-white/80 p-6 backdrop-blur-sm">
        <span className="text-xs font-medium uppercase tracking-[0.2em] text-indigo-600">
          While we wait — the RAG learning system
        </span>
        <h2 className="mt-2 text-xl font-semibold tracking-tight">
          How this system gets smarter every time someone uses it
        </h2>
        ...
        <div className="mt-4 space-y-3 text-sm leading-relaxed text-slate-700">
          ...
          <p>
            ...
            <em>retrieves the highest-rated past prompts</em> and injects them into Opus 4.7&apos;s
            system message as examples — &quot;here&apos;s what worked for past students in
            this exact situation.&quot;
          </p>
          <p>
            Opus reads those examples and <em>adapts the spirit of what worked</em> into the
            new prompt it&apos;s writing right now. It doesn&apos;t copy them verbatim — it
            uses them as taste signals.
          </p>
          <p>
            <strong>Personal layer:</strong> for signed-in users, the system also maintains a
            one-paragraph <em>preference profile</em> describing their study style. A
            background job rebuilds it every 4 hours from the student&apos;s feedback
            history. That profile gets injected as additional context so prompts feel custom
            to each person.
          </p>
        </div>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          ...
          <RagStat number="$0" label="marginal cost per training cycle" sublabel="vs. fine-tuning a model" />
        </div>

        <p className="mt-5 text-xs italic leading-relaxed text-slate-500">
          The result: a system that produces measurably better prompts on day 30 than on day
          1, without me writing any new code. It learns from collective feedback (what works
          for everyone) AND personal feedback (what works for you specifically).
        </p>
      </section>
    </main>
  );
